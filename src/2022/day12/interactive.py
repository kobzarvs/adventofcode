...
class Renderer:
    ...
    def init_pygame():
        ...
        screen = pygame.display.set_mode((Renderer.width, Renderer.height), pygame.RESIZABLE)
    ...
    def run(self, start_key=None, slide_show_key=None):
        ...
        for ctx in self.update():
            while True:
                for event in self.event_loop():
                    Renderer.clock.tick(Renderer.fps)
                    self.renderer(Renderer.screen, Renderer.font, self.state, event, ctx=ctx)
                    if self.state == 'pause':
                        continue
                if self.state != 'pause':
                    break
    ...
    def renderer(self, scr: pygame.Surface, font: pygame.font.Font, is_pass: bool, event: pygame.event.Event, ctx=None):
        if self.state == 'pause':
            return
        ...
        self.render(scr=scr, font=font, is_pass=is_pass, event=event, pg=pygame, ctx=ctx)
